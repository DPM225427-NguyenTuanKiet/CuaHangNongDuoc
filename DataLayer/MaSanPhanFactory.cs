...
namespace CuahangNongduoc.DataLayer
{
    public class MaSanPhanFactory
    {
        ...
        {
            OleDbCommand cmd = new OleDbCommand("SELECT * FROM MA_SAN_PHAM WHERE ID_SAN_PHAM=@id AND SO_LUONG > 0 ORDER BY NGAY_HET_HAN ASC");
        ...
        {
            OleDbCommand cmd = new OleDbCommand("SELECT * FROM MA_SAN_PHAM WHERE SO_LUONG > 0 ORDER BY NGAY_HET_HAN ASC");
            m_Ds.Load(cmd);
        ...
        internal static void CapNhatSoLuong(string masp, int soLuong, OleDbTransaction transaction)
        {
            OleDbCommand cmd = new OleDbCommand("UPDATE MA_SAN_PHAM SET SO_LUONG = SO_LUONG + @so WHERE ID = @id",
                transaction.Connection, transaction);
            cmd.Parameters.Add("@so", OleDbType.Integer).Value = soLuong;
            cmd.Parameters.Add("@id", OleDbType.VarChar).Value = masp;
            cmd.ExecuteNonQuery();
        }
