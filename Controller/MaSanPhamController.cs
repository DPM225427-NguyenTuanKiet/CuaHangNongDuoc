...
namespace CuahangNongduoc.Controller
{
    ...
    {
        private readonly MaSanPhanFactory factory = new MaSanPhanFactory();
        ...
        {
            DataTable tbl = factory.LaySanPham(idMaSanPham);
            SanPham sp = null;
        ...
        {
            DataTable tbl = factory.LayMaSanPham(idMaSanPham);
            MaSanPham sp = null;
        ...
        {
            return BuildMaSanPhamList(factory.DanhsachChiTiet(id));
        }

        public IList<MaSanPham> LayDanhSachMaSanPhamTheoSanPham(string sanPhamId)
        {
            return BuildMaSanPhamList(factory.DanhsachMaSanPham(sanPhamId));
        }

        private IList<MaSanPham> BuildMaSanPhamList(DataTable tbl)
        {
            SanPhamController ctrlSanPham = new SanPhamController();
            IList<MaSanPham> ds = new List<MaSanPham>();
            foreach (DataRow row in tbl.Rows)
            {
                var sp = new MaSanPham
                {
                    ...
                    ThanhTien = Convert.ToInt64(row["DON_GIA_NHAP"]) * Convert.ToInt32(row["SO_LUONG"]),
                    ...
                    SanPham = ctrlSanPham.LaySanPham(row["ID_SAN_PHAM"].ToString())
                };
                ds.Add(sp);
            }
